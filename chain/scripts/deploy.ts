...
async function main() {
  ...
  // let user = await UserAdminProfileNFT.connect(deployer).deploy();
  // user = await user.waitForDeployment("UserAdminProfileNFT", deployer);
  ...
  // let contract = await ERC6551Registry.connect(deployer).deploy();
  // let registry = await contract.waitForDeployment();
  // console.log(`Deployed ERC6551Registry contract address: ${registry.address}`);

  // console.log(`Deployed ERC6551Registry contract address: ${registry.address}`);

  let account = await ERC6551Account.connect(deployer).deploy();
  account = await account.connect(deployer).waitForDeployment();

  let pop = await CarNFT.connect(deployer).deploy();
  pop = await pop.connect(deployer).waitForDeployment();
