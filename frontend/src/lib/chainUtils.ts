import { ComethProvider, RelayTransactionResponse } from "@cometh/connect-sdk";
...
export const getRegistryContract = async (provider: ComethProvider) => {
  const contract = new ethers.Contract(
...
export const createTBAAccount = async (
  ...
  const _registryContract = await getRegistryContract(provider);
  return _registryContract.createAccount(
...
export const getTBAAccount = async (
  provider: ComethProvider,
  tokenId: number
): Promise<RelayTransactionResponse> => {
  let args: AccountCreationArgs = {
  ...
  const _registryContract = await getRegistryContract(provider);
  return _registryContract.account(
    args.implementation_,
    args.chainId_,
    args.tokenContract_,
    args.tokenId_,
